function fh = gfx_apply(frac,marg,varargin)
...
switch lower(style)
    ...
    case {'pptw','pptw_body'} %slightly different margins for full screen (no title, logos, etc.)
...
if numel(marg) == 4
    ...
    for i2 = 1:n_rows
        ...
        [~,font_height] = gfx_fontsize(ahs(j2).YAxis(yal).Label.String,ahs(j2).YAxis(yal).Label.FontSize,ahs(j2).YAxis(yal).Label.FontName,ahs(j2).YAxis(yal).Label.Units);
    ...
    for i2 = 1:n_cols
        ...
        if ~isempty(ahs(j2).Title.String) % if there's a title, margin is from top of plot to top of title
            [~,font_height] = gfx_fontsize(ahs(j2).Title.String,ahs(j2).Title.FontSize,ahs(j2).Title.FontName,ahs(j2).Title.Units);
            ...
            try 
                [~,font_height] = get_fontsize(ahs(j2).YTickLabels(end),ahs(j2).YLabel.FontSize,ahs(j2).YLabel.FontName,ahs(j2).YLabel.Units);
            catch
                font_height = 0;
            end
    ...
    for i2 = 1:n_cols
        ...
        [~,font_height] = gfx_fontsize(ahs(j2).XLabel.String,ahs(j2).XLabel.FontSize,ahs(j2).XLabel.FontName,ahs(j2).XLabel.Units);
    ...
    for i2 = 1:n_rows
        ...
        if ~isnan(yar)
            [~,font_height] = gfx_fontsize(ahs(j2).YAxis(yar).Label.String,ahs(j2).YAxis(yar).Label.FontSize,ahs(j2).YAxis(yar).Label.FontName,ahs(j2).YAxis(yar).Label.Units);
            m_right_temp(i2,2) = ahs(j2).YAxis(yar).Label.Position(1) - ahs(j2).Position(3) + font_height;
        end
        
        try
            [font_width,~] = gfx_fontsize(ahs(j2).XTickLabels(end),ahs(j2).XLabel.FontSize,ahs(j2).XLabel.FontName,ahs(j2).XLabel.Units);
        catch
            font_width = 0;
        end
...
for ii = 1:n_rows
    for iii = 1:n_cols
        ...
        for i4 = 1:numel(ahs(n).YAxis)
            ...
            if ylp(i4,1) > 0 % if it's on the right, subtract plot width
                ahs(n).YAxis(i4).Label.Position(1) = yls(i4) + box_width;
            else
                ahs(n).YAxis(i4).Label.Position(1) = yls(i4);
            end
        ...
        for i4 = 1:numel(ahs(n).XAxis)
            ...
            switch lower(xlabel_placement)
                ...
                case {'l','left','w','west'}
                    ahs(n).XAsis(i4).Label.HorizontalAlignment = 'left';
                    ahs(n).XAxis(i4).Label.Position(1) = box_width;
                case {'r','right','e','east'}
                    ahs(n).XAsis(i4).Label.HorizontalAlignment = 'right';
