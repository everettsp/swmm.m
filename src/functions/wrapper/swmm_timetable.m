function [tts,msg] = swmm_timetable(file_swmm_out,name_classes,name_elmnts)
...
for i2 = 1:numel(name_classes)
    ...
    if ~any(contains(content','<<< '))
        msg = 'SWMM run unsuccessful, ensure all parameters are within range and configured correctly';
        error(msg);
    end
