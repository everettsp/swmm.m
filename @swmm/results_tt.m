...
% load results report; if read fails, retry n times
% sometimes it fails on server, or when results stored in Google Drive
% (results file gets locked while syncing)
fid = -1;
fid_counter = 1;
while fid == -1
    fid = fopen(obj.rpt);
    fid_counter = fid_counter + 1;
    if fid_counter > 1000; break; end
end
