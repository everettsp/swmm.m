classdef uncertainty_envelope < handle
    ...
    methods
        function obj = uncertainty_envelope(data, domain, varargin)
            ...
            obj.MedPlot = plot(domain,nanmedian(data,2),args.MedStyle, 'MarkerSize', args.MarkerSize);
            obj.MeanPlot = plot(domain,nanmean(data,2),args.MeanStyle, 'MarkerSize', args.MarkerSize);
            ...
            if args.Discontinuous
                ...
                data_bounds = prctile(data,[obj.Percentile,100-obj.Percentile],2);
                obj.EnvelopePlots{1} = fill([domain(:); flipud(domain(:))],...
                    [data_bounds(:,1); flipud(data_bounds(:,2))],...
                    [0.5, 0.5, 0.5]);
            end
            ...
            if ~isempty(outliers)
                obj.OutlierPlot = plot([outliers{:,1}],[outliers{:,2}],'+','LineWidth',0.5,'MarkerSize',args.MarkerSize/4,'HandleVisibility','off');
            else
            ...
            obj.HandleVisibility = args.HandleVisibility;
        ...
        function set.HandleVisibility(obj, onoff)
            if strcmp(onoff,'off')
                obj.MeanPlot.HandleVisibility = 'off';
                obj.MedPlot.HandleVisibility = 'off';
            end
        ...
        function onoff = get.HandleVisibility(obj)
            onoff = obj.MedPlot.HandleVisibility;
        end
    end
    ...
    methods (Access=private)
        function results = check_inputs(obj, data, domain, varargin)
            ...
            p.addParameter('Discontinuous', false, @islogical);
            ...
            p.addParameter('MarkerSize',10, isscalarnumeric)
            ...
            p.addParameter('HandleVisibility','on', @ischar);
