function squeeze_ticks(obj,axis,p)
...
switch lower(axis)
    case 'y'
        for i2 = 1:numel(obj.ahs)
            lim_range = obj.ahs(i2).YLim(2) - obj.ahs(i2).YLim(1);
            obj.ahs(i2).YLim(2) = obj.ahs(i2).YLim(2) + lim_range * p;
            obj.ahs(i2).YLim(1) = obj.ahs(i2).YLim(1) - lim_range * p;
        end
    case 'x'
        for i2 = 1:numel(obj.ahs)
            lim_range = obj.ahs(i2).XLim(2) - obj.ahs(i2).XLim(1);
            obj.ahs(i2).XLim(2) = obj.ahs(i2).XLim(2) + lim_range * p;
            obj.ahs(i2).XLim(1) = obj.ahs(i2).XLim(1) - lim_range * p;
        end
    case {'xy','all','both'}
