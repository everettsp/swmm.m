function [xsynth, ysynth] = smoter(x,y,p,k,varargin)
%
...
par = inputParser;
addParameter(par,'n_synth',numel(y) - 2 * numel(r))
parse(par,varargin{:})
n_synth = par.Results.n_synth;

synth = nan(n_synth,nf);
...
for i2 = 1:n_synth
        ...
        diff = d(nn_inds(ri,ki),:) - d(r(ri),:); % diff. between rare sample and random nearest neighbour
        gap = rand; % random perm
        synth(k2,:) = d(r(ri),:) + gap .* diff;
        k2 = k2 + 1;
%     end
end
