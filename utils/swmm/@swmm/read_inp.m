...
for i2 = 1:height(class_table)
    ...
    switch classname
        ...
        case {'report'}
            ...
            y = struct();
            for i3 = 1:numel(lines)
                val = data_cell_subs(i3,2:end);
                val = val(~cellfun(@isempty,val));
                y.(strtrim(data_cell_subs{i3,1})) = val;
                clear val
            end
        case {'options'}
            ...
            for i3 = 1:numel(lid_names) % each unique LID control
                ...
                for i4 = 2:numel(data_cell_subs) % lid layers
                    ...
                    if numel(fields(y.(lid_type))) == 1
                        ...
                        if strcmp(lid_param,'removals') % special case for weird BOD/TSS removal formatting... code is pretty ugly but should work
                            vals2 = cell(1,4);
                            if any(contains(cellfun(@num2str,vals,'UniformOutput',false),'BOD'))
                                j2 = find(contains(cellfun(@num2str,vals,'UniformOutput',false),'BOD'));
                                vals2(1:2) = vals(j2+(0:1));
                            end
                            if any(contains(cellfun(@num2str,vals,'UniformOutput',false),'TSS'))
                                j2 = find(contains(cellfun(@num2str,vals,'UniformOutput',false),'TSS'));
                                vals2(3:4) = vals(j2+(0:1));
                            end
                            y.(lid_type)(k3(type_idx)).(lid_param) = cell2struct(vals2',attributes');
                            clear vars vals2 j2
                        else
